      ...
      <!-- Siparişler Başlangıcı -->
        ...
        <div class="container-xxl py-5">
                ...
                <?php
                    ...
                    echo "<th>Yemekler</th>";
                    echo "<th>Adetler</th>";
                    echo "<th>Yemek notları</th>";
                    echo "</tr>";

                    include("baglanti.php");

                        $secimidsorgu = "SELECT MIN(secimlerID) as min_secimlerID FROM secimlerID";
                        $sonucsecimid = mysqli_query($baglan, $secimidsorgu);
                        $row = mysqli_fetch_assoc($sonucsecimid);
                        $secimlerID = $row['min_secimlerID'];


                    $sorgu = "SELECT * FROM yemeksecimleri yemekler,kullanicilar kullanicilar WHERE yemekler.secimlerID = kullanicilar.secimlerID";

                    // tablo altına sipariş ürünlerinin yazdırılması için çalışan kod parçası
                    
                    $kaydetSonuc = $baglan->query($sorgu);

                    while ($sonuc = $kaydetSonuc->fetch_assoc()) {
                      echo "<tr>";
                      echo "<td>" . $sonuc["kullaniciAdSoyad"] . "</td>";
                      echo "<td>" . $sonuc["kullaniciMasaNo"] . "</td>";
                      echo "<td>" . $sonuc["kullaniciTelNo"] . "</td>";
